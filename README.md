...
如果你要更保守、尽量降低风控风险，推荐只保留 Cloudflare Worker 作为自动触发器，并把探测频率控制在每 30 分钟一次。
...
- API 可用性探测：跟 workflow 一样，每 30 分钟一次
...
- 在可接受的实时性下尽量降低调用频率
- 避免后台额度接口没必要被每 30 分钟之外的额外重试反复拉取
- 避免再依赖容易填错的换算配置
...
- 红：这个 API 入口当前探针失败
- 页面会同时显示最近更新时间，避免把低频探测误当成秒级实时状态
